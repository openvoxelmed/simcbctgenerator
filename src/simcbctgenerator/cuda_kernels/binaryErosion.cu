...
__global__ void binaryErosion(unsigned char *mask, unsigned char *maskOut, unsigned char *kernel, int sizeDimX, int sizeDimY, int sizeDimZ)
{
    ...
    int totalKernelSum = 0;
    for (int i=0; i<27; i++)
        totalKernelSum += kernelCoefficients[i];
    ...
    unsigned char binaryOutputValue = (unsigned char)(count>=totalKernelSum);
